...
async function load() {
    try {
        const [pRes, cRes] = await Promise.all([
            window.axios.get(`/api/v1/seller/products/${props.id}`),
            window.axios.get('/api/v1/admin/categories'),
        ]);
        const d = pRes.data.data ?? pRes.data;
...
async function submit() {
    ...
    try {
        ...
        await window.axios.post(`/api/v1/seller/products/${props.id}`, fd, {
...
<template>
    <Head title="Edit Product | Seller" />
    <SellerLayout>
        <div class="max-w-2xl">
            ...
            <form v-else-if="form" @submit.prevent="submit" class="space-y-5">
                ...
                <div v-if="existingImages.length" class="bg-white rounded-xl shadow-sm p-5 space-y-3">
                    <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">Current Images</h2>
                    <div class="flex flex-wrap gap-3">
                        <img v-for="(img, i) in existingImages" :key="i" :src="img.url ?? img"
                            class="w-20 h-20 object-cover rounded-lg border border-gray-200" />
                    </div>
                </div>

                <div class="bg-white rounded-xl shadow-sm p-5 space-y-3">
                    <h2 class="text-sm font-semibold text-gray-700 uppercase tracking-wide">Add New Images</h2>
